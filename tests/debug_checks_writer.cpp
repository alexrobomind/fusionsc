...
namespace goldfish
{
	...
	{
		stream::vector_writer output;
		auto array = json::create_writer(stream::ref(output), debug_checks::throw_on_error{}).write_array();
	...
	{
		stream::vector_writer output;
		auto writer = json::create_writer(stream::ref(output), debug_checks::throw_on_error{}).write_array();
