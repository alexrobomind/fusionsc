...
namespace goldfish
{
	TEST_CASE(test_filtered_map_empty_map)
	{
		...
		auto map = apply_schema(json::read(stream::read_string_literal("{}")).as_map(), schema);
	...
	TEST_CASE(test_filtered_map)
	{
		...
		auto map = apply_schema(json::read(
			stream::read_string_literal("{\"10\":1,\"15\":2,\"a\":\"b\",\"40\":3,\"50\":4,\"60\":5,\"80\":6}")).as_map(), schema);
	...
	TEST_CASE(filtered_map_skip_while_on_value)
	{
		...
		auto map = apply_schema(json::read(stream::read_string_literal("{\"20\":1}")).as_map(), schema);
	...
	TEST_CASE(test_filtered_map_by_value)
	{
		...
		auto map = apply_schema(json::read(stream::read_string_literal("{\"B\":1}")).as_map(), schema);
