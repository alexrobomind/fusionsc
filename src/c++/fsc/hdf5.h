...
namespace fsc {
	...
	H5::DataSet createDataSet(H5::Location&, kj::StringPtr, const H5::DataType&, kj::ArrayPtr<H5Dim> = nullptr);
	H5::DataSet createDimension(H5::Location&, kj::StringPtr, const H5::DataType&, const H5Dim&);
	...
	template<typename T>
	H5::DataSet createDataSet(H5::Location&, kj::StringPtr, kj::ArrayPtr<H5Dim> = nullptr);
	
	template<typename T>
	H5::DataSet createDimension(H5::Location&, kj::StringPtr, const H5Dim&);
	...
	template<typename T>
	void writeArray(const H5::DataSet&, const kj::ArrayPtr<T>& data);
...
namespace fsc {
	namespace internal {
		...
		struct H5TypeFor<cType> { \
			static const H5::PredType& value = H5::hdfType; \
		};
	...
	template<typename T>
	H5::DataSet createDataSet(H5::Location& loc, kj::StringPtr name, kj::ArrayPtr<H5Dim> dims) {
		return createDataSet(loc, name, h5Type<T>(), dims);
	}
	
	template<typename T>
	H5::DataSet createDimension(H5::Location& loc, kj::StringPtr name, const H5Dim& dim) {
	...
	T readScalar(const H5::DataSet& ds) {
		...
		T buffer;
		ds.read(&buffer, hdf5NativeType<T>(), H5::DataSpace());
	...
	template<typename T>
	Array<T> writeArray(const H5::DataSet& ds, const ArrayPtr<T>& data) {
