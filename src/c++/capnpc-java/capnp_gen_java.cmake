function(CAPNP_GEN_JAVA TARGET_NAME)
  if(NOT ARGN)
    message(SEND_ERROR "CAPNP_GEN_JAVA() called without any source files.")
  endif()
  ...
  foreach(schema_file ${ARGN})
    ...
    set(SYMBOLIC_FILE_NAME "${output_base}-java-src-symbolic")
    add_custom_command(
      OUTPUT "${SYMBOLIC_FILE_NAME}"
      COMMAND "${CAPNP_EXECUTABLE}"
      ARGS compile
          -o $<TARGET_FILE:capnpc-java>${output_dir}
          --src-prefix ${CAPNPC_SRC_PREFIX}
          ${include_path}
          ${CAPNPC_FLAGS}
          ${file_path}
      DEPENDS "${schema_file}" ${tool_depends} capnpc-java
      COMMENT "Compiling Cap'n Proto schema ${schema_file} (Java sources)"
      VERBATIM
    )
    list(APPEND DEPENDENCIES "${SYMBOLIC_FILE_NAME}")
	set_source_files_properties("${SYMBOLIC_FILE_NAME}" PROPERTIES SYMBOLIC "true")
  endforeach()
  
  add_custom_target(
    ${TARGET_NAME}
	COMMAND ${CMAKE_COMMAND} -E echo "The java classes have been generated in ${CAPNPC_OUTPUT_DIR}"
